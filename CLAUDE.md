...
**多使用设计模式，追求易维护、易扩展、低耦合的代码。避免重复造轮子、写出烂代码。**

### 核心准则

| 原则 | 说明 |
|------|------|
| **第一性原理** | 将问题拆解到最本质的基本事实，从零推理，而非依赖类比或既有方案 |
| **Unix 哲学** | 做好一件事、可组合、小模块、文本流作为通用接口 |
| **DRY（Don't Repeat Yourself）** | 相同逻辑不写第二遍，抽取为共享函数/Hook/组件 |
| **单一职责** | 一个模块/类/函数只做一件事，职责边界清晰 |
| **开闭原则** | 对扩展开放，对修改关闭——新增功能优先通过扩展而非修改现有代码 |
| **依赖倒置** | 依赖抽象（接口/类型）而非具体实现，便于替换和测试 |
| **组合优于继承** | 优先用组合（composition）复用逻辑，避免深层继承链 |

### 实践要点

- **第一性原理思考**：遇到问题时先追问"本质是什么"，不被现有实现束缚；理解根本约束后再设计方案，而非直接照搬他人做法
- **小而专注的模块**：每个模块/函数只做一件事并做到极致，通过组合而非膨胀来构建复杂系统——输出可作为输入的模块，而非一头巨兽
- **优先复用已有轮子**：写代码前先检查 `packages/shared`、`packages/ui`、项目内是否已有现成方案，不要重复实现
- **关注点分离**：数据获取（service）、状态管理（store）、UI 渲染（component）各司其职，禁止在组件中直接写 API 调用
- **面向接口编程**：模块间通过明确的 TypeScript 接口/类型通信，不依赖对方的内部实现细节
- **依赖注入**：Store 之间通过构造函数注入依赖（参考 `instances.ts`），避免模块内部 hard-code 依赖
- **策略模式**：当出现大量 `if/else` 或 `switch` 分支判断同类行为时，考虑用策略模式替代
- **写易测试的代码**：纯函数优先、副作用隔离到 `effects/`、Store 不直接操作 DOM
